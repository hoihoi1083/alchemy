 ...
 *
 * Legacy alias: POST /api/generate-kling-storyboard (re-exports this handler).
 */
export async function POST(request: Request) {
  ...
  async function runH3AndReturn(balanceAfter: number | null, tokensCharged: number) {
    console.info(
      `[generate-storyboard-video] MiniMax H3 first (${imageUrls.length} stills, ${videoUrls.length} ref video(s))`,
    );
  ...
  if (skipToKling) {
    ...
    try {
      console.info(
        `[generate-storyboard-video] Kling by wallet choice (${imageUrls.length} image(s))`,
      );
  ...
  if (runSeedanceFirst) {
    try {
      console.info(
        `[generate-storyboard-video] Seedance R2V first (${imageUrls.length} stills + ${videoUrls.length} ref video(s), quality)`,
      );
  ...
  try {
    console.info(
      `[generate-storyboard-video] Kling fallback (${imageUrls.length} image(s))`,
    );
    const result = await runKlingStoryboardFallback({
      request,
      clerkId,
