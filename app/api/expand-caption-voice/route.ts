...
function parseCaptionLines(raw: unknown): CaptionLine[] {
  ...
  return raw
    .map((row) => {
      ...
      return { startSec, endSec, text } satisfies CaptionLine;
    })
...
export async function POST(request: Request) {
  ...
  const voiceoverScript = body.voiceover_script?.trim() ?? "";
  if (captionLines.length < 1 && !voiceoverScript) {
    return NextResponse.json(
      { error: "caption_lines or voiceover_script is required." },
      { status: 400 },
    );
  }
  ...
  try {
    const result = await expandCaptionVoice({
      ...
      videoDurationSec: body.video_duration_sec,
