...
export function LandingPricingTeaser() {
  ...
  const cards = [
    ...
    },
    {
      ...
      priceLabel:
        interval === "monthly" ? P.plans.standard.monthlyPrice : P.plans.standard.yearlyPrice,
      listPrice: P.plans.standard.listPrice,
      saveLabel:
        interval === "monthly" ? P.plans.standard.monthlySave : P.plans.standard.yearlySave,
    ...
    },
    {
      ...
      priceLabel:
        interval === "monthly" ? P.plans.master.monthlyPrice : P.plans.master.yearlyPrice,
              ...
              >
                ...
                <div className="mt-4">
                  ...
                  <p className="text-2xl font-bold text-slate-900">
                    {card.priceLabel}
                    {card.id !== "free" && card.id !== "custom" ? (
                      <span className="text-xs font-medium text-slate-500">{P.perMonth}</span>
                    ) : null}
                  </p>
                  {"saveLabel" in card && card.saveLabel ? (
                    <p className="mt-0.5 text-[10px] font-medium text-violet-600">{card.saveLabel}</p>
                  ) : null}
                  {interval === "yearly" && card.id !== "free" && card.id !== "custom" ? (
                ...
                <ul className="mt-4 flex-1 space-y-2">
                  {card.features.map((f) => (
                    <li key={f} className="flex gap-1.5 text-[11px] leading-snug text-slate-600">
