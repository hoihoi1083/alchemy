...
const STRATEGY_LABEL_KEY: Record<
	ReferenceStrategyKind,
	keyof Messages["wizard"]["referenceStrategyKind"]
> = {
	none: "none",
...
const LAYER_LABEL_KEY: Record<
	keyof ReturnType<typeof layerPlanLabels>,
	keyof Messages["wizard"]["referenceLayerLabel"]
> = {
...
const ACTION_LABEL_KEY: Record<
	string,
	keyof Messages["wizard"]["referenceLayerAction"]
> = {
	keep: "keep",
	adapt: "adapt",
	replace: "replace",
...
export function ReferenceBriefPanel({ m, brief, strategy, busy, note }: Props) {
	...
	const strategyLabel =
		m.wizard.referenceStrategyKind[STRATEGY_LABEL_KEY[strategy.kind]];
				...
				<>
					...
					{brief?.layoutStyle ? (
						<p className="mt-1 text-xs text-violet-200/80">
							{m.wizard.referenceBriefLayoutDetected}:{" "}
							{brief.layoutStyle}
						</p>
					) : null}
					{brief?.colorPalette ? (
						<p className="mt-1 text-xs text-violet-200/80">
							{m.wizard.referenceBriefColors}:{" "}
							{brief.colorPalette}
						</p>
					) : null}
					{brief?.optimizedScenePrompt || brief?.sceneEssay ? (
						<p className="mt-1 text-xs text-violet-200/80">
							{m.wizard.referenceBriefSceneSpine}:{" "}
							{(
								brief.optimizedScenePrompt ||
								brief.sceneEssay ||
								""
							).slice(0, 180)}
							{(brief.optimizedScenePrompt || brief.sceneEssay || "")
						...
						<div>
							...
							<ul className="mt-1 space-y-0.5 text-xs text-violet-100/90">
								{borrow.map((key) => (
									<li key={key}>
										{
											m.wizard.referenceLayerLabel[
												LAYER_LABEL_KEY[key]
											]
										}{" "}
										·{" "}
										{
											m.wizard.referenceLayerAction[
												ACTION_LABEL_KEY[
													strategy.layers[key]
												]
											]
					...
					<p className="mt-2 text-[11px] text-violet-300/80">
						{m.wizard.referenceBriefFootnote}
					</p>
				</>
			)}
			{note ? (
				<p className="mt-2 text-xs text-violet-200/70">{note}</p>
			) : null}
		</div>
	);
}
