...
export function ImageStep() {
  ...
  const imageTokenCost = estimateImageJobTokens({
...
<section className="space-y-5 rounded-3xl border border-slate-800 bg-slate-950/70 p-5 shadow-xl shadow-slate-900/40 backdrop-blur">
  ...
  {useReferenceVideo && referenceAd && referenceIsVideo && !isStoryboardOutput && workflowMode === "combined" && (
    <div className="rounded-xl border border-amber-500/50 bg-amber-950/30 px-4 py-3 text-xs text-amber-100">
      ...
      >
        ...
        {storyboardLoading ? (
          <span className="ml-1.5 opacity-90">({m.account.checkingPlan})</span>
        ) : !storyboardAllowed ? (
  ...
  {!usesCompositor && !isStoryboardOutput && !isCinematicStitchOutput && !isConceptCinematicSingleOutput && effectiveImageMode !== "describe" && !shipItMode && (
    ...
    <ImageOutputModePicker
      ...
      lockedSingle={lockedSingleImageMode}
    />
    <div className="mt-3">
  ...
  {isStoryboardOutput && (
    <div className="rounded-2xl border border-teal-800/50 bg-teal-950/20 p-4">
      ...
      {lockStoryboardRecipeForResearch ? (
        <ResearchStoryboardModeSummary variant="dark" />
      ) : (
  ...
  {!usesCompositor && !isStoryboardOutput && !shipItMode && videoCreativeMode !== "motion-poster" && (
    <ImageTextModePicker
      value={imageTextMode}
      disabled={imageBusy}
    ...
    >
      {imageBusy
        ...
        : isStoryboardOutput && storyboardScenes.length > 0
          ...
          : (isCampaignOutput || isCinematicStitchOutput) &&
              (campaignSlides.length > 0 || cinematicScenes.length > 0)
  ...
  {isStoryboardOutput && storyboardScenes.length > 0 ? (
    <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-teal-700/70 bg-teal-950/40 px-4 py-3">
      <input
        type="checkbox"
        className="mt-1 h-4 w-4 shrink-0 rounded border-teal-400 text-teal-600"
