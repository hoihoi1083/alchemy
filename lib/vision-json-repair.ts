...
export async function runFlorenceDetailedCaption(imageUrl: string): Promise<string> {
...
export async function captionImageToVisionJson<T extends object>(input: {
  ...
  if (!deepSeekApiKey()) {
    ...
    throw new Error("DeepSeek is required to structure Florence captions into a brief.");
  }
  const raw = await callDeepSeekChat(
    ...
    { temperature: 0.2, max_tokens: 900, jsonObject: true },
  );
