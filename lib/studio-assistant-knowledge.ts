...
方案大約：Free 註冊 1000／480p；Standard 每月 3000／720p；Pro 8000／1080p；Master 16000 + Pro 畫布。付費可加購 1000 tokens。詳情 /pricing。數字係估算。`,
  ...
  },
  {
    ...
    en: `Default product/concept TVC is 4 beats (establish → macro/metaphor → orbit → payoff), shown as a 2×2 shot map (九宫格-style review, not always 9 generates). Tap every cell, then check “I looked at every cell” before Generate video. Regen one bad cell — not regen-all. Approval clears if a still changes. Video animates mistakes; fix stills first.`,
    zh: `預設產品／概念 TVC 係 4 拍（開場→微距／隱喻→環繞→收束），2×2 分鏡圖（九宮格式檢視，唔等於一定出 9 張）。要逐格點開，再剔「我睇晒每一格」先可以出片。壞格只 regen 嗰一格，唔好全部重出。靜圖一改就要重新批核。片會放大靜圖錯誤，先修好先出片。`,
  },
  {
    id: "motion-poster",
    title: "Motion poster",
    ...
    en: `Motion poster = 1 still + short micro camera/env motion (~6s). Product: usually your upload. Concept: 1 AI scene still first, then motion — never skip the still. Usually fits the free grant (~595). Landing recipe cards are hidden; start from /studio (concept + motion poster) or templates.`,
    zh: `動態海報 = 1 張靜圖 + 短微運鏡／環境動態（約 6 秒）。實體多數用你上傳嘅相。概念要先出 1 張 AI 場景靜圖再做動態 — 唔可以冇靜圖就出片。通常免費額度夠（約 595）。首頁配方卡已隱藏；由 /studio（概念 + 動態海報）或模板開始。`,
  },
  {
    id: "identity",
