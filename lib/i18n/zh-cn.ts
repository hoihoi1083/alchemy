...
export const zhCn = {
  ...
  landing: {
    ...
    toolEditTitle: "图片编辑",
    toolEditDesc: "AI 智能图层 — 移动文字／物件、改文案、导出。",
    toolSmartLayersTitle: "AI 智能图层",
    toolSmartLayersDesc: "检测文字与物件、拖动图层、笔刷抠图 — 所有方案可用。",
...
socialDripFitTitle: "这个格式可以／不可以做什么",
  ...
  editImage2: {
    ...
    magicChatHint:
      "用白话改整张图 — 不用先拆层。或：外扩 9:16 · 改成新文案（要先选文字层）",
    magicChatHintWithCosts: (c: { edit: number; expand: number }) =>
      `描述整张图怎么改（约 ${c.edit} 点）。外扩约 ${c.expand} 点。拆层是独立按钮，这里不用。免费：提起、粘贴、可编辑文字。`,
    magicChatPlaceholder: "例如：背景改夜晚 · 标题改成夏季大促…",
    magicChatSend: "执行",
    magicChatHelp:
      "描述整张图的修改。可选：外扩 1:1/9:16 · 改成 …（选中文字层）· 改这层：…（只改选中块）",
