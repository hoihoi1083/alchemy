...
"material-letters": {
  ...
  captions2: {
    ...
    bgmDragHint:
      "拖动 BGM 整块可平移；左右绿边可拉长短。点一次「添加 BGM」写入成片；写入后音轨会复位，避免重复叠加。",
    bgmStartNote: (s: number) => `混音中 BGM 从 ${s.toFixed(1)}s 开始`,
    exportPlate: "烘焙时间轴",
    pictureClear: "清除",
    pictureUploadFailed: "上传失败",
    signInToUse: "请登录后再上传与编辑视频。",
  ...
  editImage2: {
    ...
    sandwichSplitting: "混合拆层 — 先文字，再主体，再其余…",
    sandwichReady:
      "已就绪 — 文字在上、主体、组件 + 已填底图。可自由拖动。",
    sandwichReadyPartial: (n: number) =>
      `已就绪 — ${n} 层（文字＋主体）。底图已清洞 — 选一层再拖。剩余图形未拆，可用框选提起。`,
    sandwichPartialTokens:
      "文字与主体已抽出，但其余拆层未跑（点数不足）。余额足够后再试混合拆层，或用框选提起。",
    sandwichNoObjects:
      "文字与主体已就绪；没有额外组件。Logo 可能仍在底图 — 用框选／拆选区。",
    sandwichHow:
      "混合：抽字 → 主体 → 补其余（不用烂修洞做底图）。",
    sandwichAdvanced: "混合拆层",
    sandwichAdvancedHow:
      "文字 + 主体 + 其余。难图优先用这个。",
    hybridSplit: (n: number) => `混合拆层 ${n}`,
    hybridSplitHow:
      "海报首选：抽可改文字、切主体、重建底图＋剩余图形。不再用烂修洞。",
    liftTools: "提起与编辑",
    ...
    florenceSamReady: (layers: number, sam: number) =>
      `已就绪 — ${layers} 层（${sam} 个切割）。文字与组件已提起；底图已修补。`,
    florenceMatchReady: (layers: number) =>
      `已就绪 — ${layers} 层。画面与原图一致；拖动一件才会清洞。`,
    florenceSamMatchReady: (layers: number, sam: number) =>
      `已就绪 — ${layers} 层（${sam} 个切割）。画面与原图一致；拖动一件才会清洞。`,
    florenceSubjectReady: (layers: number, sam: number) =>
      `已就绪 — ${layers} 层（主体${sam ? ` + ${sam} 切割` : ""}）。主角是完整一层；画面与原图，拖动才清洞。`,
    florenceSplitHow: "文字 · 主体 · 图标。拆完与原图一样，拖动才清洞。",
    qwenSplitHow:
      "试验：一次拆成 RGBA 图层 + 已填底图。难图可与主拆层对比。",
    ...
    detectAll: "拆层",
    detectAllQwen: (n: number) => `拆层 ${n}`,
    detectAllFlorence: (n: number) => `拆层 ${n}`,
    qwenFullSplit: (n: number) => `拆层 ${n}`,
    ...
    seedreamCopyrightBlocked:
      "此图片被版权／品牌／肖像过滤拦截。点数已退回。用户上传和 AI 海报在有 Logo／角色时都可能触发。请用框选提起／清底图，或换一张图。",
    healFallbackBlur:
      "背景填充使用了快速模糊兜底（AI 填充不可用）。导出前请检查空洞 — 可撤销，或用抠边修边。",
    imageUnprocessable: "无法将此图拆成图层。请换一张，或用框选提起／清底图。",
    envNotReady: "智能拆层暂时不可用。请稍后再试，或用框选提起处理单块。",
