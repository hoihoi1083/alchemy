...
export const zhTw = {
  ...
  landing: {
    ...
    toolEditTitle: "圖片編輯",
    toolEditDesc: "AI 智慧圖層 — 移動文字／物件、改文案、匯出。",
    toolSmartLayersTitle: "AI 智慧圖層",
    toolSmartLayersDesc: "偵測文字與物件、拖動圖層、筆刷抠圖 — 所有方案可用。",
...
socialDripFitTitle: "這個格式可以／不可以做什麼",
  ...
  editImage2: {
    ...
    magicChatHint:
      "用白話改整張圖 — 不用先拆層。或：外擴 9:16 · 改成新文案（要先選文字層）",
    magicChatHintWithCosts: (c: { edit: number; expand: number }) =>
      `描述整張圖怎麼改（約 ${c.edit} 點）。外擴約 ${c.expand} 點。拆層是獨立按鈕，這裡不用。免費：提起、貼上、可編輯文字。`,
    magicChatPlaceholder: "例如：背景改夜晚 · 標題改成夏季大促…",
    magicChatSend: "執行",
    magicChatHelp:
      "描述整張圖的修改。可選：外擴 1:1/9:16 · 改成 …（選中文字層）· 改這層：…（只改選中塊）",
