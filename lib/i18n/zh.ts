...
export const zh = {
	...
	landing: {
		...
		toolEditTitle: "圖片編輯",
		toolEditDesc: "AI 智能圖層 — 移動文字／物件、改文案、匯出。",
		toolSmartLayersTitle: "AI 智能圖層",
		toolSmartLayersDesc: "偵測文字同物件、拖動圖層、筆刷抠圖 — 所有方案可用。",
...
socialDripFitTitle: "呢個格式可以／唔可以做咩",
	...
	editImage2: {
		...
		magicChatHint:
			"用白話改整張圖 — 唔使先拆層。或：外擴 9:16 · 改成新文案（要先選文字層）",
		magicChatHintWithCosts: (c: { edit: number; expand: number }) =>
			`描述整張圖要點改（約 ${c.edit} 點）。外擴約 ${c.expand} 點。拆層係獨立按鈕，呢度唔使。免費：提起、貼上、可編輯文字。`,
		magicChatPlaceholder: "例如：背景改夜晚 · 標題改成夏季大促…",
		magicChatSend: "執行",
		magicChatHelp:
			"描述整張圖嘅修改。可選：外擴 1:1/9:16 · 改成 …（選中文字層）· 改這層：…（只改選中塊）",
