...
export const zh = {
  ...
  footer: {
    tagline: "上傳產品相片，幾分鐘生成吸睛社交廣告 — 圖片、影片、配樂一次完成。",
    productTitle: "產品",
    studio: "開始製作",
    pricing: "方案與 tokens",
    how: "運作方式",
  ...
  start: {
    ...
    welcomeTitle: "歡迎加入！",
    welcomeBody: "你已獲得 {n} 免費 tokens，可以開始創作。請先選擇下方路徑。",
  },
  header: {
  ...
  pricing: {
    ...
    tokenItems: [
      ...
      {
        title: "對應真實 AI 用量",
        body: "費用反映 fal.ai 圖片/影片同規劃 API — 唔係虛擬點數。",
      },
      {
        title: "加購唔使升級",
    ...
    faq: [
      ...
      {
        q: "生成失敗會扣 token 嗎？",
        body: "唔會 — AI 任務失敗會釋放預留 token 回餘額。",
      },
      {
        q: "免費計劃可以做商業用途嗎？",
        body: "可以試用完整流程。商業大量使用建議升級付費計劃以獲更高解像度同限額。",
      },
      {
        q: "幾時可以買額外 token？",
    ...
    comparisonRows: [
      ...
      { feature: "圖片最高解像度", free: "1K", standard: "1K", pro: "1K", master: "2K" },
      { feature: "影片最高解像度", free: "480p", standard: "720p", pro: "1080p", master: "1080p" },
      { feature: "平台研究 / 合併流程 / 故事板", free: "✓", standard: "✓", pro: "✓", master: "✓" },
      { feature: "A/B、Campaign、教學輪播", free: "✓", standard: "✓", pro: "✓", master: "✓" },
      { feature: "電郵支援", free: "—", standard: "✓", pro: "✓", master: "✓" },
      { feature: "優先生成", free: "—", standard: "—", pro: "✓", master: "✓" },
      { feature: "Pro 畫布", free: "—", standard: "—", pro: "—", master: "✓" },
      { feature: "優先支援", free: "—", standard: "—", pro: "—", master: "✓" },
      { feature: "加購 token（$10 / 1k）", free: "—", standard: "✓", pro: "✓", master: "✓" },
    ],
  },
  steps: {
    setup: "設定",
  ...
  wizard: {
    ...
    storyboardPlanReviewHint:
      "先用 DeepSeek 出分鏡大綱，改到啱再生成場景圖。唔啱嘅場景描述／畫面字可以直接改。",
    storyboardPlanBtn: "生成分鏡大綱（DeepSeek）",
    storyboardPlanBusy: "DeepSeek 規劃中…",
    ...
    pathQuickVideoDesc: "一張產品相 + AI 寫 Seedance 運鏡 prompt → 短宣傳片。",
    ...
    closestMatchRecipeHint:
      "圖片→影片流程：3 張 AI 關鍵幀 + 3 段 Seedance 片段拼接（約 24 秒）。先出無聲片 — 完成後再加 BGM 同字幕。",
    ...
    conceptCinematicSingleVideoStepIntro:
      "8 秒 Reel：撳「生成完整 Reel」會做 1 段 Seedance（無聲片 — 完成後再加 BGM／字幕）。",
    ...
    conceptCinematicSingleRecipeSteps: [
      ...
      "2) Step 3 撳「生成完整 Reel」— 出無聲 Seedance 片。",
    ...
    cinematicLogoStampNote: "已喺出片前將品牌 logo 印喺靜幀（每場景同一位置同大小）。",
    cinematicLogoStampHint:
      "喺 Brand kit 上傳你嘅 logo — 出片前會印喺每張電影靜幀（預設右上、約 11% 闊），每個場景同一位置。",
    cinematicStitchWorkflowOrder:
      "生成順序：{count} 段 Seedance → ffmpeg 拼接。BGM 同字幕可喺完成後加。腳本／音樂可選用廣告包或字幕工作室。",
    ...
    cinematicStitchRecipeSteps: [
      ...
      "2) Step 3 直接撳「生成完整 Reel」— Seedance + 拼接（無聲）。",
      "3) 完成後下載無聲 MP4，或開啟字幕同音頻工作室。",
    ],
    cinematicStitchVideoStepIntro:
      "{count} 場景拼接：撳「生成完整 Reel」會自動做 {count} 段 Seedance + 拼接（BGM／字幕之後可選）。",
    ...
    researchReelMp4OptionalCombined:
      "圖文帖唔使 MP4（選填）— 會用參考圖風格 + DeepSeek 分鏡出片。",
    ...
    researchReelPickDurationFirst:
      "請先喺上方揀輸出片長（唔好留「自動」），參考片分析同 Seedance 出片先會按呢個長度規劃。",
    researchReelReanalyzeForDuration: "片長已改 — 重新分析參考片…",
    researchReelStatusOutputDuration: "輸出片長已揀（分析同計費用）",
    researchReelStatusOutputDurationMissing: "請先揀輸出片長（4–12 秒）— 參考片可以好長，出片長度以呢度為準",
    ...
    brandKit: {
      title: "品牌套件",
      hint: "顏色同標語會引導 AI 畫面風格。Logo 請喺出圖後自己加（快速小修或畫布）。",
      uploadLogo: "上傳 Logo",
      changeLogo: "更換 Logo",
      primaryColor: "主色",
      secondaryColor: "副色",
    ...
    videoSetupOutputSettingsHint:
      "fal.ai 按 Seedance 片長同解像度計費 — 請喺上傳參考片／分析前先揀。參考 MP4 可以好長；實際出片以呢度嘅片長為準（AI 會將參考節奏壓縮成完整短片）。",
    ...
    step4Hint: "下載無聲 MP4（未加 BGM），或開啟音頻／字幕工作室自行加音樂同字幕。",
    ...
    planVideoPromptDurationRefresh: "片長已改 — DeepSeek 按新片長重寫影片 prompt…",
  ...
  errors: {
    ...
    insufficientTokens: "Token 不足，無法生成。請到 Pricing 升級或加購後再試。",
    ...
    falContentPolicy:
      "Seedance 拒絕呢次媒體（人物／私隱過濾）。我哋可以改用 Kling 逐場動畫再拼接——如果自動切換失敗，請再撳生成影片。",
    klingStoryboardFailed: "Kling 分鏡備援失敗，請再試或改用無臉場景圖。",
    needPhoto: "請先上傳產品相片。",
  ...
  ugcStudio: {
    ...
    productHowHint:
      "兩個模式 — 我嘅關鍵幀圖：用你嘅產品相生成手持／手腕口播靜態圖，再交俾 HeyGen 對嘴。庫存主播：通用數字人，產品名只會喺講稿講出嚟，唔會顯示你嘅真產品。",
    productLabel: "產品名稱",
    ...
    planScript: "AI 寫講稿（DeepSeek）",
    planningScript: "寫講稿中…",
    planScriptHint: "用 DeepSeek 根據產品名草稿約 10 秒口播。生成後可以再改。",
    scriptReady: "AI 講稿已好 — 可改再預聽聲線。",
    scriptFailed: "AI 講稿失敗。請檢查 DeepSeek API key。",
    localeHk: "粵語/繁中",
    localeCn: "普通话",
    localeEn: "English",
    voiceLabel: "聲線",
    avatarVoiceLabel: "主播聲線（跟呢個主播鎖定）",
    avatarVoiceHint: "每個庫存主播都有配對嘅 MiniMax 聲線（跟你揀嘅語言）。",
    previewVoice: "預聽聲線",
    previewingVoice: "預聽中…",
    ...
    download: "下載 MP4",
    costHint: "會扣 token（聲線 + HeyGen 影片，約 30 token/秒音訊 ≈ $0.10/秒）。需要登入。",
    needScript: "請先填口播講稿。",
    needProduct: "請先填產品名稱。",
  ...
  captions: {
    ...
    planCaptionVoiceFailed: "AI 規劃失敗。請檢查 DeepSeek API key。",
    planCaptionVoiceDone:
      "已規劃 {n} 句短字幕 + 加長口播（約 {sec}s）— 下一步：預聽 → 混入口播（{n} 段）。",
    audioBgmLabel: "音樂曲目",
    audioApplyBgm: "加 BGM",
    ...
    audioVoiceDoneAtCaption: "口播已按自然語速混入，從字幕開始時間 {sec}s 起播（唔拉長唔壓縮）。",
    audioVoiceDonePerCaption:
      "已用自然語速按字幕時段混入 {n} 段口播（有「口播」欄就讀口播；唔拉長唔壓縮）。",
    audioVoicePerCaptionHint:
      "畫面字幕保持短句。口播可以加長填滿每段時段 — 或撳「加長口播配合時段」— 再預聽 → 混入。連續預聽可以短過片長；混入會按每段口播放落對應時段。",
    audioVoiceNeedCaptionLines:
      "字幕時間軸至少要有 2 句有文字嘅字幕（而家只有 {n} 句）。請由口播稿同步，或喺右邊改字幕，再混入。",
    audioVoiceNeedPreviewOrScript: "請先生成口播預聽（或填口播稿）揀一把聲。",
    ...
    expandCaptionVoiceFailed: "加長口播失敗。請檢查 DeepSeek API key。",
    ...
    beatStatusReady: "已偵測 {n} 個節拍 — 開吸附拖動，或撳「對齊字幕到節拍」。",
    beatStatusEmpty: "未偵測到節拍 — 請先加 BGM，等系統再分析。",
    beatStatusUnavailable: "節拍偵測需要伺服器影片網址（請加 BGM 或由工作室開啟）。",
  ...
  microWizard: {
    ...
    generateImageFooterHint: "揀出圖數量（單張 / A/B / Campaign / 教學輪播），然後按下方「生成相片」。",
    referenceAnalyzeReadyTitle: "參考圖分析完成",
    referenceAnalyzeReadyHint: "分析完成 — 會自動進入下一步；亦可按「繼續」。",
    ...
    videoModeHint: "Seedance 點樣用你嘅關鍵幀或參考片？",
    imagePromptTitle: "圖片潤色 prompt",
    imagePromptHint: "生成前可選填 — 俾 Nano Banana 嘅額外指示。",
    ...
    primaryStyleHint: "先揀快速廣告或模特兒佩戴／使用。UGC 同參考排版屬影片或其他進階路徑。",
    productNameTitle: "產品名稱",
    productNameHint: "賣緊乜？會用作標題同 prompt 錨點。",
    conceptTitle: "概念助手",
    conceptHint: "描述服務或優惠 — 非實體 SKU。",
    conceptSourceTitle: "概念從哪裡開始？",
    conceptSourceHint: "揀一個起點 — AI 概念簡報或平台研究參考，唔會同時混用。",
    conceptSourceAssistantTitle: "概念助手",
    conceptSourceAssistantDesc: "由 AI 幫你寫文案同視覺方向 — 跳過平台研究。",
    conceptSourceResearchTitle: "平台研究",
    conceptSourceResearchDesc: "搵 trending 排版參考 — 跳過概念助手，之後必須檢查文案。",
    conceptTopicTitle: "研究主題",
    conceptTopicHint: "你推廣嘅服務或概念 — 用作平台研究搜尋錨點，唔會跑概念助手。",
    conceptTopicLabel: "概念 / 服務主題",
    ...
    videoSubpathTitle: "影片創作風格",
    videoSubpathHint: "推廣片、參考短片，或 UGC 數字人口播。",
    refVideoTitle: "參考短片（MP4）",
    refVideoHint: "有 reel／短片先上傳；圖文研究帖可跳過（用參考圖 + 分鏡即可）。",
    refVideoTitleOptional: "參考短片（選填）",
    refVideoHintOptional: "圖文帖唔使上傳 MP4 — 會自動跳過。有 reel 先上傳會跟節奏排分鏡。",
