...
"material-letters": {
	...
	captions2: {
		...
		bgmDragHint:
			"拖 BGM 整塊可平移；左右綠邊可拉長短。撳一次「加 BGM」寫入成片；寫入後音軌會復位，避免重複疊加。",
		bgmStartNote: (s: number) => `混音入面 BGM 由 ${s.toFixed(1)}s 開始`,
		exportPlate: "烘焙時間軸",
		pictureClear: "清除",
		pictureUploadFailed: "上傳失敗",
		signInToUse: "請先登入再上傳同編輯影片。",
	...
	editImage2: {
		...
		sandwichSplitting: "混合拆層 — 先文字，再主體，再其餘…",
		sandwichReady:
			"已就緒 — 文字在上、主體、組件 + 已填底圖。可自由拖動。",
		sandwichReadyPartial: (n: number) =>
			`已就緒 — ${n} 層（文字＋主體）。底圖已清洞 — 選一層再拖。剩餘圖形未拆，可用框選提起。`,
		sandwichPartialTokens:
			"文字與主體已抽出，但其餘拆層未跑（點數不足）。餘額足夠後再試混合拆層，或用框選提起。",
		sandwichNoObjects:
			"文字與主體已就緒；沒有額外組件。Logo 可能仍在底圖 — 用框選／拆選區。",
		sandwichHow:
			"混合：抽字 → 主體 → 補其餘（唔用爛修洞做底圖）。",
		sandwichAdvanced: "混合拆層",
		sandwichAdvancedHow:
			"文字 + 主體 + 其餘。難圖優先用呢個。",
		hybridSplit: (n: number) => `混合拆層 ${n}`,
		hybridSplitHow:
			"海報首選：抽可改文字、切主體、重建底圖＋剩餘圖形。唔再用爛修洞。",
		liftTools: "提起同編輯",
		...
		florenceSamReady: (layers: number, sam: number) =>
			`已就緒 — ${layers} 層（${sam} 個切割）。文字同組件已提起；底圖已修補。`,
		florenceMatchReady: (layers: number) =>
			`已就緒 — ${layers} 層。畫面同原圖一致；拖動一件先會清洞。`,
		florenceSamMatchReady: (layers: number, sam: number) =>
			`已就緒 — ${layers} 層（${sam} 個切割）。畫面同原圖一致；拖動一件先會清洞。`,
		florenceSubjectReady: (layers: number, sam: number) =>
			`已就緒 — ${layers} 層（主體${sam ? ` + ${sam} 切割` : ""}）。主角係完整一層；畫面同原圖，拖動先清洞。`,
		florenceSplitHow: "文字 · 主體 · 圖標。拆完同原圖一樣，拖動先清洞。",
		qwenSplitHow:
			"試驗：一次拆成 RGBA 圖層 + 已填底圖。難圖可同主拆層對比。",
		...
		detectAll: "拆層",
		detectAllQwen: (n: number) => `拆層 ${n}`,
		detectAllFlorence: (n: number) => `拆層 ${n}`,
		qwenFullSplit: (n: number) => `拆層 ${n}`,
		...
		seedreamCopyrightBlocked:
			"呢張圖被版權／品牌／肖像過濾攔截。點數已退回。用戶上傳同 AI 海報有 Logo／角色時都可能踩到。請用框選提起／清底圖，或換一張圖。",
		healFallbackBlur:
			"背景填充用咗快速模糊兜底（AI 填充唔得）。匯出前檢查吓窿 — 可以撤銷，或者用抠邊修邊。",
		imageUnprocessable: "拆唔到圖層。試吓換圖，或者用框選提起／清底圖。",
		envNotReady: "智能拆層暫時唔用得。遲啲再試，或者用框選提起處理單塊。",
