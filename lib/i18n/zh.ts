...
export const zh = {
  ...
  account: {
    title: "帳戶與帳單",
    subtitle: "查看計劃、點數餘額同帳單紀錄。需要時可喺 Stripe 管理信用卡同發票。",
    loading: "載入帳戶中…",
    loadError: "無法載入帳戶。",
    planLabel: "目前計劃",
  ...
  start: {
    ...
    phasesImage: ["揀推廣類型", "揀創作路徑", "素材同設定", "生成圖片", "檢查同匯出"],
    phasesVideo: ["揀推廣類型", "揀創作路徑", "影片簡報", "生成影片", "檢查同匯出"],
    phasesCombined: ["揀推廣類型", "揀創作路徑", "分鏡設定", "分鏡圖", "製作影片"],
    phaseBodies: [
      "揀實體產品定服務／概念。",
      "只要相片、只要影片，或先圖再片。",
      "填資料、研究、風格同素材。",
  ...
  landing: {
    ...
    navCaptions: "字幕",
    navPro: "Pro",
    heroTrust: ["唔使空白 prompt", "先分鏡再出片", "產品同概念都得", "AI 市場研究"],
  ...
  pricing: {
    ...
    faq: [
      ...
      },
      {
        q: "點樣升級計劃？",
        body:
          "登入後喺呢個收費頁揀更高計劃（Standard → Pro → Master）確認即可。升級會由今日起開新帳單期：Stripe 退回舊計劃未用時間嘅差額，再由今日起收新計劃費用 — 我哋唔會改 Stripe 產品標價。你保留剩餘 token，並即時獲得新計劃完整份額（例如 Standard 3,000 → Master 會再加 16,000）。下次續費由升級日起重新計算（月付／年付取決於你揀嘅計劃）。升級唔使經「管理帳單」。",
      },
      {
        q: "點樣降級計劃？",
        body:
          "喺收費頁揀較低計劃。降級會排程到下一個帳單日生效 — 之前你仍然用緊目前計劃、功能同剩餘點數。較低價錢同計劃點數由下個週期開始。咁樣可以避免中途用低價享用高計劃權益。「管理帳單」用嚟取消續訂／改卡，唔係揀較低計劃。",
      },
      {
        q: "喺邊度改計劃或取消訂閱？",
        body:
          "升級或排程降級：登入後喺收費頁操作。取消續訂或更新付款方式：用「管理帳單」（Stripe）。取消後，目前週期完結前仍然可用付費功能。",
      },
    ],
    footnote:
  ...
  wizard: {
    ...
    creationPath: {
      ...
      nextStepsImage: [
        { title: "設定", body: "參考、產品同圖片設定。", icon: "setup" },
        { title: "生成圖片", body: "為活動產出靜態圖。", icon: "image" },
        { title: "檢查同匯出", body: "喺畫布編輯，再下載。", icon: "done" },
      ],
      nextStepsVideo: [
        { title: "影片簡報", body: "產品、動態方向同設定。", icon: "setup" },
        { title: "生成影片", body: "準備好就生成短片。", icon: "video" },
        { title: "檢查同匯出", body: "預覽、字幕同下載。", icon: "done" },
      ],
      nextStepsCombined: [
        { title: "分鏡設定", body: "簡報、場景同靜態圖設定。", icon: "setup" },
        { title: "分鏡圖", body: "檢查場景圖（未係最終影片）。", icon: "image" },
    ...
    visualStyles: {
      ...
      "creative-video": {
        title: "創意動態簡報",
        description: "用文字描述你想拍嘅 Reel → AI 寫動態 prompt（運鏡／節奏）",
      },
      "concept-cinematic": {
        title: "概念電影感短片",
    ...
    storyboardVideoPreflight: "有研究片：Seedance R2V → H3。淨靜幀：H3 → Kling 拼接",
    ...
    researchReelCopyingNote: "跟緊你嘅參考片 — 如果人臉被擋會改用 MiniMax H3。",
    ...
    pathQuickVideoDesc: "一張產品相 + AI 寫運鏡 prompt → 短宣傳片（先出無聲片，之後可加 BGM／字幕）。",
    ...
    conceptVideoCreativeMode: {
      title: "概念影片（由文案出片）",
      description: "用 Concept Wizard 填好嘅內容，唔使產品相。先寫 AI 動態 prompt，再生成。",
    },
    ...
    imageReviewFailedBody: "請睇下面錯誤再試。若提示未扣費，今次唔會扣 tokens。",
    ...
    videoReviewHeroHint: "預覽、下載無聲片，或開啟字幕工作室加 BGM 同畫面文字。",
    ...
    videoReviewFailedBody: "請睇下面錯誤，再返回影片設定。若提示未扣費，今次唔會扣 tokens。",
    ...
    productVideoPlanHint: "已分析你上傳嘅相；AI 寫好鏡頭同動態。可喺進階修改。",
    productVideoAssistantPreflight: "模式：產品動態助手 — 多圖 reference-to-video",
    productVideoAnalyzeFirstHint: "上傳主產品 → 撳「分析相片並寫動態 Prompt」→ 再生成。",
    productVideoUploadFirstHint: "請先上傳主產品相片，再撳「分析相片並寫動態 Prompt」。",
  ...
  errors: {
    ...
    creativeVideoPromptRequired: "請先撳「AI 寫動態 Prompt」並檢查下方 prompt。",
  ...
  imageCanvas: {
    ...
    stepHowToClean: "可選：擦除 Logo 或文字，再用呢張圖加自己文案 — 唔使清就跳過。",
    ...
    cleanMultiRegionHint: "可圈選最多 5 個區域。只會改你塗嘅位，唔會成個櫃／產品一齊刪。",
    ...
    cleanPromptPlaceholder: "替換例子：改成「認識金砂石」／大理石枱面（擦走唔使寫）",
    cleanEraseBtn: "擦走塗抹區域",
    cleanFillBtn: "按描述替換",
    cleanCostNote: "每次擦走／替換消耗 1 次圖片額度。AI 只改你塗紫色嘅像素。",
  ...
  studioAssistant: {
    ...
    welcomeSite:
      "你好 — 我可以幫你開 wizard、修圖、字幕、Pro 畫布或品牌套件。想做咩？",
    shortLabel: "問 AI",
  ...
  microWizard: {
    ...
    preVideoSetup: {
      ...
      conceptPhotoHint: "選填 — 幫畫面落地；概念短片亦可以只靠簡報 + 動態 prompt。",
      brandTitle: "品牌網站",
