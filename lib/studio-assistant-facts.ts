...
function askFormatRule(): string {
  return [
    ...
    "Real paths you may mention as markdown: [/](/) [/start](/start) [/studio](/studio) [/captions](/captions) [/edit-image-2](/edit-image-2) [/ultra](/ultra) [/brand-kit](/brand-kit) [/library](/library) [/ugc](/ugc) [/pricing](/pricing).",
...
- /start：实体 vs 概念。/studio 引导 wizard（无聊天助理，跟屏幕步骤）。/captions 烧字幕。/edit-image-2 修图（智能图层）。/ultra Ultra 画布（Master）。
...
- /start：實體 vs 概念。/studio 引導 wizard（無聊天助理，跟屏幕步驟）。/captions 燒字幕。/edit-image-2 修圖（智能圖層）。/ultra Ultra 畫布（Master）。
...
- /start: physical vs concept. /studio guided wizard (no chat assistant — follow on-screen steps). /captions burn-in. /edit-image-2 retouch (smart layers). /ultra Ultra canvas (Master).
