...
export type UserReferenceBrief = {
	...
	mood: string;
	motionHints: string;
	/** 反推 screenplay of the reference (pre-SKU swap). */
	sceneEssay?: string;
	/** 优化 screenplay after swapping to the user product + campaign copy. */
	optimizedScenePrompt?: string;
	/** product|headline|subline|offer used for optimizedScenePrompt. */
...
export function briefFromCarouselVision(
	...
	return {
		...
		sceneEssay: [
			...
			vision.sharedLayoutFamily
				? `Layout family: ${vision.sharedLayoutFamily}`
				: "",
			vision.sharedColorPalette ? `Palette: ${vision.sharedColorPalette}` : "",
			vision.sharedMood ? `Mood: ${vision.sharedMood}` : "",
			vision.slides[0]?.sceneSummary
				? `Cover: ${vision.slides[0].sceneSummary}`
				: "",
		]
			.filter(Boolean)
			.join(". "),
...
export function briefFromUserTextOnly(input: {
	...
	return {
		...
		contentSummary: [conceptIdea, headline, subline]
			.filter(Boolean)
			.join(". "),
		visibleText: "",
		subjects: "",
...
export function isInfographicLikeBrief(
	brief: UserReferenceBrief | string | undefined,
): boolean {
	const text =
		typeof brief === "string" ? brief : userReferencePromptBlock(brief!);
...
export function isPhotographicReferenceBrief(
	...
	if (
		/lifestyle-photo|product-ad|product photo|photograph|photography|flat lay/i.test(
			blob,
		)
	) {
		return true;
...
export function userReferencePromptBlock(brief: UserReferenceBrief): string {
	...
	if (brief.visibleText)
		parts.push(`On-image text to preserve: ${brief.visibleText}`);
	...
	if (brief.typographyStyle)
		parts.push(`Typography: ${brief.typographyStyle}`);
	...
	if (brief.carouselSlides?.length) {
		...
		parts.push(
			carouselSlidesPlannerBlock(brief.carouselSlides).replace(
				/\n/g,
				" | ",
			),
		);
...
function layerActionHint(layer: string, action: string): string | null {
	...
	if (action === "replace")
		return `${layer}: REPLACE with user product & copy`;
...
export function userReferenceLayoutTransferPromptBlock(
	...
	if (brief.layoutStyle)
		parts.push(`Reference layout grammar: ${brief.layoutStyle}`);
	if (brief.colorPalette)
		parts.push(`Reference colors: ${brief.colorPalette}`);
	if (brief.typographyStyle)
		parts.push(`Reference typography: ${brief.typographyStyle}`);
	...
	if (brief.optimizedScenePrompt || brief.sceneEssay) {
		parts.push(
			`SCENE ESSAY (optimized look-copy — IMAGE 1 pixels are the only hero; product name is claim only; ignore any other product named here): ${brief.optimizedScenePrompt || brief.sceneEssay}`,
		);
	}
	if (brief.carouselSlides?.length) {
		parts.push(
...
export function toStyleOnlyReferenceExtra(fullExtra: string): string {
	...
	let s = fullExtra
		...
		.replace(
			/^Subjects:.*$/m,
			"Subject themes (original characters, same roles):",
		)
		.replace(
			/^On-image text to preserve:/m,
			"Reference text themes (rephrase per slide):",
		);
...
export function isLayoutTransferReferenceExtra(
...
export function mergeUserReferenceBrief(
	...
	return {
		...
		sceneEssay: fromVision.sceneEssay || fromText.sceneEssay,
		optimizedScenePrompt:
			fromVision.optimizedScenePrompt || fromText.optimizedScenePrompt,
		optimizeFingerprint:
			fromVision.optimizeFingerprint || fromText.optimizeFingerprint,
	};
}
...
export function overrideBriefForContentResearch(
	...
	return {
		...
		subjects: brief.subjects
			...
			: "",
		userConceptIdea:
			userInputs.conceptIdea?.trim() || brief.userConceptIdea,
		userHeadline: headline || undefined,
		userSubline: subline || undefined,
		sceneEssay: brief.sceneEssay,
		optimizedScenePrompt: undefined,
		optimizeFingerprint: undefined,
	};
}
