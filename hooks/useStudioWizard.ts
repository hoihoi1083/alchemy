...
import { brandProfilePromptBlock, type BrandProfile } from "@/lib/brand-profile";
...
export function useStudioWizard(promotionMode: PromotionMode) {
  ...
  const {
    ...
    videoBgmEnabled, setVideoBgmEnabled,
    ...
    conceptPlanBusy, setConceptPlanBusy,
    ...
    storyboardGridApproved, setStoryboardGridApproved,
    storyboardCellsViewed, setStoryboardCellsViewed,
  ...
  const lastMotionPosterDialectRef = useRef<MotionPosterDialectId | null>(null);
  ...
  }, [storyboardSceneSig, setStoryboardGridApproved, setStoryboardCellsViewed]);
      ...
      if (videoCreativeMode === "motion-poster") {
        ...
        setVideoPrompt(
          buildMotionPosterPrompt({
            ...identity,
            durationSec,
            mode: endFramePhoto || endFrameUrl ? "start-end" : "loop",
            dialect: previewMotionPosterDialect(),
          }),
        );
  ...
  const analyzeResearchReel = useCallback(
    async (videoFile: File): Promise<boolean> => {
      ...
      try {
        ...
        if (typeof data.referenceVideoUrl === "string" && data.referenceVideoUrl.startsWith("http")) {
  ...
  useEffect(() => {
    ...
    if (researchReelAnalyzeInFlightKeyRef.current === reelAnalyzeCacheKey) return;
  ...
  }, [productPhoto, packagingPhoto, extraKitPhotos, m.wizard.planStaleAfterAssetChange]);
      ...
      if (
        ...
        setVideoPromptPlanNote((prev) =>
          prev?.includes(m.wizard.planVideoPromptDurationRefresh)
            ? prev
            : [prev, m.wizard.planVideoPromptDurationStale].filter(Boolean).join(" — "),
        );
      }
      return;
    }
  ...
  function applyLandingRecipe(recipeId: LandingRecipeId) {
    ...
    if (def.duration) {
      setVideoSettings((s) => ({
        ...
        autoSecondFrame: isPoster ? false : s.autoSecondFrame,
        videoEngine: isPoster ? "minimax-h3" : s.videoEngine,
      }));
      if (isTvc) {
        setStoryboardTrimDuration("12");
      }
    }
    if (def.videoCreativeMode) {
  ...
  async function refreshImagePostflight(url: string) {
    ...
    // Motion poster still is an intermediate keyframe on the video path — skip Bagel QA
  ...
  }, [workflowMode, visualStyleId, imageOutputMode, shipItMode, useOriginalImage, videoCreativeMode]);
  ...
  function applyGeneratedImages(urls: string[], endpoint?: string): string | null {
    ...
    // Postflight/Bagel runs from the imageUrl effect — do not call again here.
    savePromptSnapshot(
  ...
  function onVideoCreativeModeChange(mode: VideoCreativeMode) {
    ...
    if (mode === "motion-poster") {
      ...
      setVideoSettings((s: VideoSettings) => ({
        ...s,
        duration: s.duration === "auto" || Number(s.duration) > 8 ? "6" : s.duration,
        autoSecondFrame: false,
        motionStyle: "gentle-orbit",
  ...
  const motionPosterCanAutoStill =
    ...
    (Boolean(productPhoto) ||
      (promotionMode === "concept" &&
        Boolean(conceptIdea.trim() || headline.trim() || product.trim())));
  ...
  const formatEta = useCallback(
    (sec: number): string => {
      ...
      if (s >= 60) {
        ...
        return (m.wizard.progressEtaMinutes ?? m.wizard.progressEta).replace(
          "{minutes}",
          String(minutes),
  ...
  function canGenerateImage(): boolean {
    if (promotionMode === "concept" && videoCreativeMode === "motion-poster") {
      return Boolean(
        conceptIdea.trim() ||
          headline.trim() ||
          product.trim() ||
  ...
  async function refineResearchVideoScriptIfNeeded(): Promise<{
    ...
    else if (imageUrl?.startsWith("http")) fd.set("product_photo_url", imageUrl);

    const res = await fetch("/api/refine-research-video-script", { method: "POST", body: fd });
    const data = await res.json();
    if (!res.ok) {
    ...
    const nextAnalysis = {
      ...
      motionSummary:
        String(data.motionSummary ?? "").trim() || baselineAnalysis.motionSummary,
      productionNotesZh:
  ...
  async function makeReferenceVideo(refVideo: File): Promise<string> {
    ...
    if (useH3) {
      ...
      if (productPhoto) fd.append("reference_images", productPhoto);
      else if (imageUrl) fd.set("image_start_url", imageUrl);
    }
    const res = await fetch(useH3 ? "/api/generate-minimax-h3" : "/api/generate", {
      method: "POST",
      body: fd,
    });
    const data = await res.json();
    ...
    const usedFallback =
      ...
      String(data.generationMode ?? "").startsWith("minimax-h3-fallback") ||
  ...
  async function makeKlingStoryboardVideo(
    ...
    if (!res.ok) {
      ...
      if (code === "INSUFFICIENT_TOKENS" && data.hint === "tvc_needs_paid_plan") {
    ...
    notifyCreditBalance(readCreditBalanceFromResponse(data));
    const usedSeedance = String(data.generationMode ?? "").includes("seedance");
    const usedH3 = String(data.generationMode ?? "").startsWith("minimax-h3");
    ...
    const timingManifest = videoDurationSec
      ? buildSingleClipManifest(videoDurationSec, {
          ...
          engine: usedSeedance ? "seedance" : usedH3 ? "unknown" : "kling",
          timingSource: Number(data.outputDurationSec) > 0 ? "probed" : "reported",
        })
      : null;
    if (timingManifest) {
  ...
  async function makeProductAssistantVideo(): Promise<string> {
    ...
    const prompt = stripReferenceVideoTags(
      productVideoPlan?.seedancePrompt?.trim() || videoPrompt.trim() || "",
    );
    ...
    setVideoNote(
      [
        String(data.generationMode ?? "").startsWith("minimax-h3-fallback") ||
        (data.seedanceBlockedCode && String(data.generationMode ?? "").startsWith("minimax-h3"))
          ? m.wizard.seedanceToMinimaxH3FallbackNote
  ...
  async function makeMultiAngleVideo(): Promise<string> {
    ...
    fd.set(
      "prompt",
      seedancePromptForGenerate(promptWithSpine, { hasReferenceVideo: hasRefVideo }),
    );
  ...
  async function generateMotionPosterKeyframe(
    ...
    try {
      ...
      fd.set("business", business.trim());
      fd.set("headline", headline.trim() || product.trim() || conceptIdea.trim());
      fd.set("subline", subline.trim());
      fd.set("offer", offer.trim());
      fd.set("prompt_market", promptMarket);
      fd.set("subject_framing", subjectFraming);
      ...
      fd.set("workflow_mode", workflowMode);
      fd.set("promotion_mode", promotionMode);
      fd.set("image_text_mode", frame === "end" ? "integrated" : "textless");
      fd.set("aspect_ratio", effectiveImageAspectRatio);
      fd.set(
        "endpoint",
      ...
      if (frame === "end" && startPlateUrl) fd.set("start_plate_url", startPlateUrl);
      attachReferenceToForm(fd);
  ...
  async function makeMotionPosterVideo(imageStartUrlOverride?: string): Promise<string> {
    ...
    const dialectId = resolveMotionPosterDialect({
      ...
      conceptMode: promotionMode === "concept",
      excludeId:
        motionPosterDialectPick === "auto" ? lastMotionPosterDialectRef.current : null,
    }).id;
    ...
    if (override && marked && override === marked) {
      startUrl = override;
    } else if (imageUrl && !useOriginalImage && marked && imageUrl === marked) {
      startUrl = imageUrl;
    }
    if (!startUrl) {
      if (!productPhoto && promotionMode !== "concept") {
    ...
    if (endFramePhoto) {
      ...
      endUrl = await generateMotionPosterKeyframe(dialectId, "end", startUrl);
    }
    ...
    const vOpts = resolveVideoGenerationOpts(templateId, videoSettings);
    const durationSec = resolvePlannerDurationSec(String(vOpts.duration), 6);
    ...
    fd.set(
      "headline",
      headline.trim() || product.trim() || conceptIdea.trim() || business.trim(),
    );
    fd.set("subline", subline.trim());
    fd.set("offer", offer.trim());
    fd.set(
      "product_name",
      promotionMode === "concept"
        ? effectivePromoteName || product.trim() || conceptIdea.trim() || business.trim()
    ...
    const pathNote = data.generationMode
      ...
      : "";
    setVideoNote(
      [m.wizard.motionPosterHint, pathNote, data.note as string | undefined]
        .filter(Boolean)
        .join(" · "),
    );
    return data.videoUrl as string;
  ...
  async function generateVideo(opts?: { imageUrlOverride?: string }) {
    ...
    if (promotionMode === "concept" && !conceptIdentityOk && !directReferenceR2vReady) {
    ...
    if (isStoryboardOutput) {
      ...
      const clipDur = klingClipDurationForStoryboard(sceneCount, totalSec);
      const hasReel = Boolean(
        (referenceAd && referenceIsVideo) || referenceVideoFalUrl?.startsWith("http"),
      );
      ...
      const afford = evaluateStoryboardVideoAffordability({
        ...
        klingCanHitDuration: klingStitchCanHitDuration(clipDur * sceneCount, totalSec, {
          clipCount: sceneCount,
        }),
        h3Cost: videoTokenCost("720p", Math.min(15, Math.max(4, totalSec))),
        klingCost: estimateKlingStoryboardTokens(sceneCount, clipDur),
        seedanceCost: videoTokenCost("1080p", Math.min(15, Math.max(4, totalSec))),
  ...
  const videoGenerateDisabled =
    (promotionMode === "physical" &&
      videoCreativeMode === "reference-concept" &&
      !productPhoto) ||
    (!hasFinalImage && !conceptTextVideoReady && !directReferenceR2vReady && !motionPosterCanAutoStill) ||
    ...
    (isStoryboardOutput && storyboardScenes.length === 0) ||
    (isStoryboardOutput && !storyboardGridApproved) ||
